...
public sealed partial class PtyStream
{
    ...
    private void ReadLoop()
    {
        ...
        readerStarted.Set();
        WindowsPty.Diag($"reader started tid={readerThreadId} outRead={(long)outputRead.DangerousGetHandle()}");
    ...
    private bool TrySatisfyFromBuffer(ReadOperation waiter)
    {
        ...
        waiter.Completed = true;
        waiter.Offset = n; // the sync return path reads Offset (not the TCS result)
